...
import { Calendar, MapPin, ImageOff, Images } from 'lucide-react'
import { formatEventDateRange, eventVenueLabel } from '../lib/eventDisplay'

export default function PastEventCard({ event, delay = 0, onViewMedia }) {
  return (
    <div
    ...
    >
      ...
      <div className="relative w-full aspect-[16/10] overflow-hidden bg-gradient-to-br from-[#c84c30]/15 to-[#8ab4f8]/15">
        ...
        <span className="absolute top-3 right-3 inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest bg-cozy-dark/80 text-white shadow-sm">
          Completed
        </span>
      </div>
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <button
          type="button"
          onClick={onViewMedia}
          className="mt-auto w-full min-h-11 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-xs font-bold text-white bg-[#c84c30] hover:bg-[#b04027] transition-colors shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c84c30] focus-visible:ring-offset-2"
        >
          <Images className="w-3.5 h-3.5" aria-hidden="true" /> View Highlights
        </button>
      </div>
    </div>
  )
